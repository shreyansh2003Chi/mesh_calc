...
class HexagonalStaggeredTriangularHoleWpoli extends StatefulWidget {
  final MeshItem item;

  const HexagonalStaggeredTriangularHoleWpoli({super.key, required this.item});

  @override
  State<HexagonalStaggeredTriangularHoleWpoli> createState() => _HexagonalStaggeredTriangularHoleWpoliState();
}

class _HexagonalStaggeredTriangularHoleWpoliState extends State<HexagonalStaggeredTriangularHoleWpoli> {
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      Provider.of<HexagonalStaggeredTriangularHoleWpoliProvider>(context, listen: false).clearControllers();
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Consumer<HexagonalStaggeredTriangularHoleWpoliProvider>(
          builder: (context, p, child) {
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Center(
                  child: Image.asset(widget.item.image, height: 200, fit: BoxFit.contain),
                ),
                ...
                MeasurementField(
                  controller: p.hexagonalHoleCtrl,
                  label: "Hexagonal Hole (HH)",
                  unit: p.hexagonalHoleUnit,
                  onUnitChanged: p.setHexagonalHoleUnit,
                ),
                ...
                const SizedBox(height: 18),
                MeasurementField(
                  controller: p.perforationDiameterCtrl,
                  label: "Perforation Diameter (PD)",
                  unit: p.perforationDiameterUnit,
                  onUnitChanged: p.setPerforationDiameterUnit,
                ),
                const SizedBox(height: 18),
                MeasurementField(
