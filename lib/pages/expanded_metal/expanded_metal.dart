...
class ExpandedMetal extends StatefulWidget {
  final MeshItem item;

  const ExpandedMetal({super.key, required this.item});

  @override
  State<ExpandedMetal> createState() => _ExpandedMetalState();
}

class _ExpandedMetalState extends State<ExpandedMetal> {
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      Provider.of<ExpandedMetalProvider>(context, listen: false).clearControllers();
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Consumer<ExpandedMetalProvider>(
          builder: (context, p, child) {
            return Column(
              children: [
                Column(
                  children: [
                    Image.asset(AppAssets.expandedMetalCore, height: 200, fit: BoxFit.contain),
                ...
                Column(
                  children: [
                    ...
                    MeasurementField(
                      controller: p.shortWayDiagonalController,
                      label: AppString.shortWayDiagonal,
                      unit: p.shortWayDiagonalUnit,
                      onUnitChanged: p.setShortWayDiagonalUnit,
                    ),
                    const SizedBox(height: 18),
                    MeasurementField(
                      controller: p.longWayDiagonalController,
                      label: AppString.longWayDiagonal,
                      unit: p.longWayDiagonalUnit,
                      onUnitChanged: p.setLongWayDiagonalUnit,
                    ),
                    const SizedBox(height: 18),
                    MeasurementField(controller: p.strandController, label: AppString.strand, unit: p.strandUnit, onUnitChanged: p.setStrandUnit),
                    const SizedBox(height: 18),
                    MeasurementField(controller: p.thicknessController, label: AppString.thickness, unit: p.thicknessUnit, onUnitChanged: p.setThicknessUnit),
                    const SizedBox(height: 18),
                    MeasurementField(controller: p.widthController, label: AppString.widthW, unit: p.widthUnit, onUnitChanged: p.setWidthUnit),
                    const SizedBox(height: 18),
                    MeasurementField(controller: p.lengthController, label: AppString.lengthL, unit: p.lengthUnit, onUnitChanged: p.setLengthUnit),
                    const SizedBox(height: 18),
                    AppTextField().textField(p.costPerKg, AppString.costPerKg),
                  ],
                ),
