...
class RectangleWireScreen extends StatefulWidget {
  final MeshItem meshItem;

  const RectangleWireScreen({super.key, required this.meshItem});

  @override
  State<RectangleWireScreen> createState() => _RectangleWireScreenState();
}

class _RectangleWireScreenState extends State<RectangleWireScreen> {
  @override
  void initState() {
    Provider.of<RectangleWireProvider>(context, listen: false).clearController();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Consumer<RectangleWireProvider>(
          builder: (context, p, child) {
            return Column(
              children: [
                ...
                MeasurementField(controller: p.widthThicknessCtrl, label: AppString.widthWireThicknessWwt, unit: p.widthThicknessUnit, onUnitChanged: p.setWidthThicknessUnit),
                const SizedBox(height: 16),
                MeasurementField(controller: p.lengthThicknessCtrl, label: AppString.lengthWireThicknessLwt, unit: p.lengthThicknessUnit, onUnitChanged: p.setLengthThicknessUnit),
                const SizedBox(height: 16),
