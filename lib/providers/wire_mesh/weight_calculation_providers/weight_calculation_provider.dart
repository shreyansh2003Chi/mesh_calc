...
class WeightCalculationProvider with ChangeNotifier {
  ...
  void calculate() {
    ...
    // Number of wires
    numberOfLineWires = widthM / pitchWidth;
    numberOfCrossWires = lengthM / pitchLength;
    ...
    // Total volume
    final double totalVolume = widthWireVolume + lengthWireVolume;

    // Base weight in KG
    final double baseWeight = totalVolume * selectedMaterial.density;

    // Apply selected crimp percentage
    weightWithoutWastage = baseWeight * (1 + (selectedCrimpPercentage / 100));
