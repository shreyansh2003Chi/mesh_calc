...
class WeightPerOdLessIdWwProvider with ChangeNotifier {
  ...
  void setInnerDiameterUnit(MeasureUnit unit) { innerDiameterUnit = unit; notifyListeners(); }
  ...
  void calculate() {
    ...
    final double id = innerDiameterUnit.toMm(double.tryParse(innerDiameterCtrl.text) ?? 0) / 1000;
    ...
    // Calculate area of OD less ID
    final double areaOfMesh = (pi * (pow(od, 2) - pow(id, 2))) / 4;
