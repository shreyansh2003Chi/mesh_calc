...
class HexagonalStaggeredTriangularHoleWpoliProvider with ChangeNotifier {
  ...
  void setHexagonalHoleUnit(MeasureUnit u) { hexagonalHoleUnit = u; notifyListeners(); }
  ...
  Future<void> calculate() async {
    ...
    final double hexagonalHole = double.tryParse(hexagonalHoleCtrl.text.trim()) ?? 0;
    ...
    double hh = _toMm(hexagonalHole, hexagonalHoleUnit);
    double p = _toMm(pitch, pitchUnit);
    
    double holeArea = hh * hh;
    double pitchArea = p * p;
    if (pitchArea > 0) {
